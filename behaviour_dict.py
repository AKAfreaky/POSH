...
class BehaviourDict:
    ...
    def registerBehaviour(self, behaviour):
        ...
        @type behaviour: L{POSH.Behaviour}
        ...
        @raise NameError: If a given action or sense is already
            ...
            dictionary.
        """
        actions, senses = behaviour.getActions(), behaviour.getSenses()
        # add the behaviour
        behaviourName = behaviour.getName()
        if self._behaviours.has_key(behaviourName):
            raise NameError, \
                "Behaviour '%s' cannot be registered twice" % behaviourName
        self._behaviours[behaviourName] = behaviour
        # add the actions
        for action in actions:
            if self._actions.has_key(action):
                raise NameError, \
                    "Action '%s' registered twice: For '%s' and '%s'" % \
                    (action, self._actions[action][1].getName(), behaviourName)
            try:
                actionMethod = getattr(behaviour, action)
            except AttributeError:
                raise AttributeError, \
                    "Behaviour '%s' does not provide an action method named '%s'" % \
        ...
        for sense in senses:
            if self._senses.has_key(sense):
                raise NameError, \
                    ...
                    (sense, self._senses[sense][1].getName(), behaviourName)
            try:
                senseMethod = getattr(behaviour, sense)
            except AttributeError:
                raise AttributeError, \
                    "Behaviour '%s' does not provide a sense method named '%s'" % \
                    (behaviourName, sense)
    ...
    def getBehaviours(self):
        ...
        @rtype: sequence of L{POSH.Behaviour} objects
        """
    ...
    def getAction(self, actionName):
        ...
        @rtype: L{POSH.Behaviour} class method
